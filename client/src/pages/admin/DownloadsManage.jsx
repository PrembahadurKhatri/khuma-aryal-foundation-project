...
import { fetchDownloads, createDownload, updateDownload, deleteDownload } from "../../services/downloadService.js";
import FileSourceField from "../../components/admin/FileSourceField.jsx";
import useToast from "../../hooks/useToast.js";

const TYPES = ["AnnualReport", "Brochure", "PressRelease", "RegistrationForm", "Newsletter", "Other"];
const TYPE_LABELS = {
...
const emptyForm = { titleEn: "", titleNe: "", type: "Other", date: "", fileFile: null };
...
const DownloadsManage = () => {
  ...
  const { data, isLoading } = useQuery({ queryKey: ["admin-downloads"], queryFn: fetchDownloads });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, payload }) => updateDownload(id, payload),
    onSuccess: () => {
      invalidate();
  ...
  const handleDelete = async (id) => {
    if (confirm("Delete this download? This cannot be undone.")) {
  ...
  return (
    <div>
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="font-body text-2xl font-bold">Downloads</h1>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-end justify-center overflow-y-auto bg-black/60 sm:items-center sm:p-4">
          <form onSubmit={handleSubmit} className={`my-0 max-h-[92vh] w-full space-y-3 overflow-y-auto rounded-t-2xl border p-6 sm:my-8 sm:max-w-lg sm:rounded-2xl ${panelClass}`}>
            <h2 className="mb-2 font-body text-lg font-semibold">{editing ? "Edit Download" : "New Download"}</h2>
            ...
            <FileSourceField
              ...
              label="File"
              required={!editing}
              existingUrl={editing?.file}
              fileValue={form.fileFile}
              onFileChange={(f) => setForm((prev) => ({ ...prev, fileFile: f }))}
            />
