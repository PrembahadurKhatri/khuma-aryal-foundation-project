...
const VacanciesManage = () => {
  ...
  // With no feedback while a request is in flight, the admin tends to
  // click "Create"/"Save" again, firing duplicate requests. The loading
  // toast + disabled/spinner button below (isSaving) both exist to make
  // that wait visible instead of silent.
  const isSaving = createMutation.isPending || updateMutation.isPending;

  const handleSubmit = async (e) => {
    e.preventDefault();
    const loadingId = toast.loading(editing ? "Saving changes — please wait…" : "Creating — please wait…");
    try {
      if (editing) {
  ...
  return (
    <div>
      ...
      {isLoading ? (
        ...
        <div className={`rounded-xl border p-6 text-center ${panelClass} ${mutedClass}`}>No vacancies yet.</div>
      ) : (
        <>
            ...
            {data?.data?.map((item) => (
              <AdminMobileCard key={item._id} theme={theme} onEdit={() => openEdit(item)} onDelete={() => handleDelete(item._id)}>
                ...
                <p className={`mt-0.5 font-body text-xs ${mutedClass}`}>
                  {TYPE_LABELS[item.type] || item.type} · Deadline {new Date(item.deadline).toLocaleDateString()}
                </p>
              </AdminMobileCard>
            ))}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center overflow-y-auto bg-black/60 sm:items-center sm:p-4">
          <form onSubmit={handleSubmit} className={`my-0 max-h-[92vh] w-full space-y-3 overflow-y-auto rounded-t-2xl border p-6 sm:my-8 sm:max-w-lg sm:rounded-2xl ${panelClass}`}>
            ...
            <div className="flex flex-col-reverse gap-2 pt-2 sm:flex-row sm:justify-end sm:gap-3">
              ...
              >
                {isSaving && <Spinner />}
                {isSaving ? (editing ? "Saving…" : "Creating…") : editing ? "Save Changes" : "Create Vacancy"}
              </button>
            </div>
          </form>
        </div>
